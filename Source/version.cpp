...
{
	std::string response;
	if (!HttpRequest("https://api.github.com/repos/" AFU_GITHUB_REPO "/releases/latest", response))
	{
	...
	if (latest_version != AFU_VERSION)
	{
		char buffer[256];
		sprintf_s(buffer, "A new version of Revsim FPS Unlocker is available.\n\nCurrent Version: %s\nLatest Version: %s\n\nVisit download page?", AFU_VERSION, latest_version.c_str());

		if (MessageBoxA(NULL, buffer, "Update Check", MB_YESNOCANCEL | MB_ICONEXCLAMATION) == IDYES)
		{
			ShellExecuteA(NULL, "open", "https://github.com/" AFU_GITHUB_REPO "/releases", NULL, NULL, SW_SHOWNORMAL);
			return true;
		}
	}
