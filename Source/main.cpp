...
std::vector<HANDLE> GetRevsimProcesses(bool include_client = true, bool include_studio = true)
{
	std::vector<HANDLE> result;
	if (include_client)
	{
		for (HANDLE handle : ProcUtil::GetProcessesByImageName("RevsimPlayerBeta.exe")) result.emplace_back(handle);
	}
	if (include_studio) for (HANDLE handle : ProcUtil::GetProcessesByImageName("RevsimStudioBeta.exe")) result.emplace_back(handle);
	return result;
}

HANDLE GetRevsimProcess()
{
	auto processes = GetRevsimProcesses();
...
const void *FindTaskScheduler(HANDLE process, const char **error = nullptr)
{
	try
	{
		...
		while (true)
		{
			...
			{
				if (error) *error = "Failed to get process base! Please restart Revsim FPS Unlocker.";
				return nullptr;
			}
		}
...
struct RevsimProcess
{
	...
	void Tick()
	{
		...
		if (!ts_ptr)
		{
			...
			if (!ts_ptr)
			{
				...
				if (retries_left-- <= 0)
					NotifyError("revsimfpsunlocker Error", error ? error : "Unable to find TaskScheduler! This is probably due to a Revsim update-- watch the github for any patches or a fix.");
				return;
			}
		}
		...
		if (ts_ptr && !fd_ptr)
		{
			try
			{
				if (auto scheduler = (const uint8_t *)(ProcUtil::ReadPointer(handle, ts_ptr)))
				{
					...
					{
						if (retries_left-- <= 0)
							NotifyError("revsimfpsunlocker Error", "Variable scan failed! This is probably due to a Revsim update-- watch the github for any patches or a fix.");
						return;
					}
			...
			{
				printf("[%p] RevsimProcess::Tick failed: %s (%d)\n", handle, e.what(), e.GetLastError());
				if (retries_left-- <= 0)
					NotifyError("revsimfpsunlocker Error", "An exception occurred while performing the variable scan.");
			}
		}
	}
	...
	void SetFPSCap(double cap)
	{
		if (fd_ptr)
		{
			...
			{
				printf("[%p] RevsimProcess::SetFPSCap failed: %s (%d)\n", handle, e.what(), e.GetLastError());
			}
		}
	}
};

std::unordered_map<DWORD, RevsimProcess> AttachedProcesses;
...
DWORD WINAPI WatchThread(LPVOID)
{
	...
	while (1)
	{
		auto processes = GetRevsimProcesses(Settings::UnlockClient, Settings::UnlockStudio);
		...
		for (auto& process : processes)
		{
			...
			if (AttachedProcesses.find(id) == AttachedProcesses.end())
			{
				...
				RevsimProcess revsim_process;

				revsim_process.Attach(process, 3);

				AttachedProcesses[id] = revsim_process;
...
bool CheckRunning()
{
	SingletonMutex = CreateMutexA(NULL, FALSE, "AFUMutex");
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (UI::IsConsoleOnly)
	{
		...
		if (!RevsimProcess().Attach(process, 0))
		{
	...
	else
	{
		if (CheckRunning())
		{
			MessageBoxA(NULL, "Revsim FPS Unlocker is already running", "Error", MB_OK);
		}
		else
		{
